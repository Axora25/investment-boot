...
# Note: Some API keys may not have access to 'gemini-1.5-pro'. We'll try a fallback order.
# You can override with GEMINI_MODEL in your environment.
PREFERRED_GEMINI_MODELS = [
    os.getenv("GEMINI_MODEL") or "gemini-1.5-pro",
    "gemini-1.5-flash",
]

def _try_gemini_models_for_generation(prompt, generation_config):
    last_error = None
    for model_name in PREFERRED_GEMINI_MODELS:
        try:
            ...
            if response and hasattr(response, 'text') and response.text:
                app.logger.info(f"Gemini response succeeded using model: {model_name}")
                return response
        except Exception as e:
            last_error = e
            app.logger.warning(f"Gemini model '{model_name}' failed: {str(e)}")
            continue
    if last_error:
        raise last_error
    return None
...
def generate_gemini_response(prompt, max_tokens=500):
    try:
        ...
        # Generate content with proper error handling and model fallback
        response = _try_gemini_models_for_generation(prompt, generation_config)
...
# --- Twelve Data helpers ---
def get_price_from_twelvedata(symbol, api_key=TWELVEDATA_API_KEY):
    ...
    try:
        ...
        if response.status_code == 200:
            data = response.json()
            # Accept price from multiple possible fields
            price_candidates = [
                data.get("price"),
                data.get("close"),
                data.get("last"),
            ...
            app.logger.warning(f"TwelveData error {response.status_code}: {response.text[:200]}")
    except Exception as e:
...
def test_twelvedata():
    """Test if Twelve Data API key works by fetching AAPL quote."""
    test_symbol = request.args.get('symbol', 'AAPL')
    if TWELVEDATA_API_KEY is None:
        return jsonify({'success': False, 'message': 'TWELVEDATA_API_KEY not set in environment.'}), 400
    # Try to fetch price
    price = get_price_from_twelvedata(test_symbol)
    if price is not None:
        return jsonify({'success': True, 'symbol': test_symbol, 'price': price, 'message': 'Twelve Data API key is working.'})
    # On failure, include provider error body for diagnostics
    try:
        debug_resp = requests.get("https://api.twelvedata.com/quote", params={
            'symbol': test_symbol,
            'apikey': TWELVEDATA_API_KEY,
        }, timeout=6)
        debug_text = debug_resp.text
        return jsonify({
            'success': False,
            'symbol': test_symbol,
            'status_code': debug_resp.status_code,
            'provider_response': debug_text[:1000],
            'message': 'Twelve Data API key is not working or rate limited.'
        }), 400
    except Exception as e:
        return jsonify({
            'success': False,
